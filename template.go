...
func genTemplateConf(categoryName, resourceName, packageName, testDir, fileName, schemaDir, templateDocDir, examplesDir string) templateDef {
	t := templateDef{
		...
		LowerCamelName:        fixUpperCase(strcase.ToLowerCamel(resourceName)),
		CamelName:             fixUpperCase(strcase.ToCamel(resourceName)),
		...
		FullCamelResourceName: fixUpperCase(strings.ToUpper(categoryName) + strcase.ToCamel(resourceName)),
